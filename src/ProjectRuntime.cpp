...
ProjectProfile demoProfile() {
  return ProjectProfile{
      "mjpeg-video-streaming-hardware-ui",
      ...
      MetricSpec{"loop_jitter_us", "us", 500.0, true},
      MetricSpec{"fault_count", "count", 0.0, true},
      true};
}

RuntimeSample nominalSample() {
  return RuntimeSample{1U, 225.0, 0.0, true, "host-ci-nominal"};
}

RuntimeSample unsafeSample() {
  return RuntimeSample{2U, 675.0, 2.0, true, "host-ci-limit-test"};
}

RuntimeSample offlineSample() {
  return RuntimeSample{3U, 225.0, 0.0, false, "host-ci-offline"};
}
